...
export class ApiImportService {
    ...
    private async importTrainLines(trainSectionDtoDictionary: Dictionary<TrainSectionDto[]>) {
      this.dataAccess.client.$transaction(async (tx) => {
    ...
    private async importTrainRides(trainSectionDtoDictionary: Dictionary<TrainSectionDto[]>) {
      this.dataAccess.client.$transaction(async (tx) => {
    ...
    private async importTrainSections(trainSectionDtosGroupedByLine: Dictionary<TrainSectionDto[]>) {
        ...
        const chunked = ListUtils.chunk(values(trainSectionDtosGroupedByLine).flat(), 2500);
        for (let i = 0; i < chunked.length; i++) {
          await this.importTrainSectionsChunk(chunked[i], i + 1, chunked.length);
        }
    ...
    private async importTrainConnectionData(sbbTrainStopDto: SbbTrainStopDto[]) {
        ...
        await this.importTrainLines(trainSectionDtosGroupedByLine);
        await this.importTrainRides(trainSectionDtosGroupedByLine);
        await this.importTrainSections(trainSectionDtosGroupedByLine);
    }
}